...
tabela intitulada ``TV'' contém atributos como a marca do aparelho e os comandos
...
Assim, podemos recuperar os bits de referência e, posteriormente, decodificá-los
...
Como no Arduino, a BeagleBone possui muitos pinos de entrada e saída para 
...
seria pouco portável. A última solução foi introduzida com o kernel 3.8.x do
Linux embarcado, acessando as interfaces GPIO através do kernel, basicamente
atribuindo valores a arquivos. Tal processo só pode ser executado como root,
então deve se entender bem o que está fazendo antes de executar tais comandos.

Os arquivos de configuração do GPIO pelo kernel estão localizados no caminho
