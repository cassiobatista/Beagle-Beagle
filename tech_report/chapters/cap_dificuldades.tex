...
pelo photosensor, verificando o tempo que ele passa ligado e desligado e
salvando várias medidas de tempo numa matriz de inteiros, o que deixa o
...
\item Como explicado no capítulo de Revisão Teórica, um bit é representado por
duas partes, sendo uma metade nível baixo e a outra nível alto. Cada nível ocorre
num intervalo de 32 ciclos (ou 16 ciclos para no caso do protocolo RC-5), sendo o
nível alto gerado pelo protocolo por modulação PWM com \textit{duty cycle} de 25\%
do período do ciclo. 
...
isto é, para gerar a parte ativa do PWM ela respondeu com um tempo maior até que
o próprio ciclo, que demora 27.6 $\mu$s aproximadamente. Se levarmos em conta o
tempo que a BeagleBone Black leva para escrever (chamado de $t_{set}$), tem se que: 
...
tem se que:
\begin{center}
			$ 1s \rightarrow 36k valores \rightarrow 27 \frac{\mu s}{valor} $ \\
			$ 1s \rightarrow n valores \rightarrow 111.6 \frac{\mu s}{valor} $	
\end{center}		
Resolvendo a regra de 3 inversa, tem se:
\begin{center}
			$ n \approx 8.96k \rightarrow f_{coding} = 8.96 kHz $ \\
			$ \frac{f_s}{f_{coding}} \approx 4 $ 
\end{center}	
Com isso pode se observar que a frequ\^{e}ncia de amostragem seria 4 vezes menor que a
desejada. Para combater este problema, foi necess\'{a}rio abrir m\~{a}o do PWM e reajustar
