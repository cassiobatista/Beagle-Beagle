...
\begin{enumerate}
\item O Ångström é o sistema operacional de fábrica da \textit{BeagleBone Black}
$^{\scriptsize{\textregistered}}$, bastante referenciado em fóruns e candidato 
...
\item A \textit{BeagleBone Black}$^{\scriptsize{\textregistered}}$ não possui 
...
trabalhoso, seria conectar um alto-falante USB à \textit{BeagleBone Black}
$^{\scriptsize{\textregistered}}$. Em se tratando de um protótipo, um 
\textit{headphone} faz o papel de um \textit{speaker} que deve consumir pouca 
...
foi conectar um roteador \textit{wifi} à porta \textit{Ethernet} da BBB através 
de um cabo UTP.
