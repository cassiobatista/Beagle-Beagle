...
através da interface de voz. O resultado era enviado para uma SparkFun IOIO
\textit{Board}, a qual, estando fisicamente conectada ao controle da TV, 
disparava um determinado \textit{trigger}. A justificativa do trabalho era 
...
Já em~\cite{Sefat14}, um sistema inteligente de segurança foi implementado na
BeagleBone, a qual utilizava a biblioteca OpenCV para contar quantos indivíduos
encontravam-se próximo à entrada do ambiente a ser vigiado. Um módulo de
reconhecimento de fala, também basedo no PocketSphinx, foi utilizado para
receber comandos do usuário, enquanto o \textit{software} eSpeak foi encarregado 
de prover as respostas dadas pela BeagleBone via voz sintetizada. Um aparelho
...
poderiam ser dados diretamente do aparelho móvel, e sim pessoal e diretamente à
BeagleBone, a qual possuía um microfone conectado à porta USB.
...
\textit{software} que permitem que a BeagleBone ``fale''~\cite{eSpeak}.
...
conectá-lo à Internet através de um conector Ethernet. O Raspberry Pi ainda
possui interface de saída HDMI e é bastante útil para aplicações gráficas.
...
\subsection{Protocolos IR: RC-5 e RC-6}
...
documentação pelos fabricantes, que também é bastante escassa. A Philips, por
exemplo, utiliza seu próprio protocolo de comunicação, chamado RC-5. A última
documentação foi liberada em 1992, quando ainda não existiam muitos dos
equipamentos eletrônicos atuais, como \textit{home theaters} ou DVDs. Nesse
padrão, os bits são codificados de acordo com o código de Manchester, onde cada
bit, transmitido dentro de um período fixo, é representado por uma transição
\textit{high-to-low} (0) ou \textit{low-to-high} (1). Esses padrões de dados são
obtidos através de uma operação do tipo XOR (OU-Exclusivo) realizada entre o
clock do dispositivo e o dado propriamente dito~\cite{rc5}.
...
nível alto. Cada nível ocorre num intervalo de 32 períodos. O nível alto é 
gerado por um PWM de \textit{duty cycle} igual a 25\% do período do pulso. Essa
percentagem define o tempo em que o IR Led permanece aceso, ou seja, no nível
alto, o IR Led permanece ligado por período de $0,25\times1/36000~s$ e desligado
por $0,75\times1/36000~s$, sendo o processo repetido 32 vezes. A economia de 
energia é mostrada como justificativa para explicar o motivo de o led piscar ao 
invés de se manter aceso por 100\% do período do pulso.
...
de \textit{start}, é sempre mantido em valor alto. O bit de \textit{toggle}, o
qual muda de estado caso uma tecla deixe de ser pressionada, também possui uma
duração mais longa do que os outros bits comuns. O período de um bit passa de 32
para 16 pulsos, exceto para os casos especiais (AGC e \textit{toggle}). Por fim,
os últimos 16 bits representam o endereço do aparelho e o referido comando a ser
transmitido, respectivamente.

%\begin{align*}
\begin{gather*}
	\text{T}      = 16 \times \frac{1}{f=36000Hz} = 444.45~\mu s \\\notag
	\text{AGC}    = 6T + 2T = 3.556~m s \\\notag
	\text{Toggle} = 2T + 2T = 1.778~m s \\\notag
	\text{Bit}    = 1T + 1T = 888.9~\mu s \\\notag
	\text{Total}  = 1*\text{AGC} + 1*\text{Toggle} + 20*\text{Bit} = 23.11~m s \\
\end{gather*}
...
\textit{smartphones} iOS e Android (sem versão para tablets, todavia), um hub
...
RF entre o \textit{smartphone}/controle com o hub (que, infelizmente, ainda não
conseguiu se livrar do tão antiquado IR), o que faz com que o usuário não
precise apontar o controle para o dispositivo que precisa controlar. Todavia, o
hub precisa estar numa boa posição para conseguir emitir a informação de forma
...
na saída de audio \textit{jack} do \textit{smartphone}. Versões mais recentes 
já possuem o \textit{hardware} que pode ser acessado via \textit{bluetooth}, 
custando US\$ 60 o mais caro. A grande vantagemé que o IRdroid, além de ser 
